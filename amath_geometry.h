...
float distance_point_plane(const Point2 &point, const Point2 &plane_point,
                           const Vec2 &plane_normal) {
   Vec2 v = plane_point - point;
   dot_product(v, plane_normal);
}
...
Mat3 combine_transforms(std::vector<Mat3> transforms) {
   ...
   struct RegularPolygon {
      ...
      RegularPolygon(Vec2 center, int num_vertex, float scale, float rotation = 0,
